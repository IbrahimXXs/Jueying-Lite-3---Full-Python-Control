...
def set_linear_velocity_x():
    """
    Sets the linear velocity of the robot along the x-axis (m/s).
    """
    command_code = 0x0140
    command_type = 1  # Complex command
    value_range = [-1.0, 1.0]  # Allowed range

    try:
        value = float(input(f"Enter Linear Velocity along X-axis (m/s) [{value_range[0]} to {value_range[1]}]: "))
        ...
        print(f"Linear velocity along x-axis set to {value} m/s.")
    except ValueError:
