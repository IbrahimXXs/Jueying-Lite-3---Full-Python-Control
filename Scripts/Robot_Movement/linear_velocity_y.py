...
def set_linear_velocity_y():
    """
    Sets the linear velocity of the robot along the y-axis (m/s).
    """
    command_code = 0x0145
    command_type = 1  # Complex command
    value_range = [-0.5, 0.5]  # Allowed range

    try:
        value = float(input(f"Enter Linear Velocity along Y-axis (m/s) [{value_range[0]} to {value_range[1]}]: "))
        ...
        print(f"Linear velocity along y-axis set to {value} m/s.")
    except ValueError:
